...
function readDB(file) {
  try { return JSON.parse(fs.readFileSync(path.join(DATA, file), 'utf8')); } catch(e) { return file.endsWith('.json') && file.includes('lawyers') ? [] : (file.includes('documents') ? [] : {}); }
}
...
function parseBody(req, cb) {
  let b = '';
  req.on('data', c => b += c);
  req.on('end', () => { try { cb(null, JSON.parse(b)); } catch(e) { cb(e, null); } });
}
...
function serveFile(res, filename) {
  try {
    ...
    const ct = filename.endsWith('.html') ? 'text/html; charset=utf-8' : 'application/javascript';
    res.writeHead(200, { 'Content-Type': ct });
    res.end(content);
  } catch(e) { res.writeHead(404); res.end('File not found: ' + filename); }
}
...
function startServer(apiKey) {
  const server = http.createServer((req, res) => {
    ...
    if (req.method === 'GET') {
      ...
      if (url === '/test') {
        groqRequest(apiKey, 'You are helpful.', [{ role:'user', content:'Say OK.' }], (t, e) => {
          ...
          if (e) res.end('<h2 style="color:red;font-family:sans-serif;padding:30px">❌ Key Error: ' + e + '<br><br><a href="https://console.groq.com">Get a new key</a></h2>');
          else res.end('<h2 style="color:green;font-family:sans-serif;padding:30px">✅ Working! AI said: ' + t + '<br><br><a href="/">Open WakeelAI</a></h2>');
        });
        return;
      }
    }
    ...
    // ── API: get lawyers (for main chat to recommend) ────────
    if (req.method === 'GET' && url === '/api/lawyers') {
      const lawyers = readDB('lawyers.json');
      const cat = req.url.includes('?') ? new URLSearchParams(req.url.split('?')[1]).get('cat') : null;
      const active = lawyers.filter(l => l.active && (!cat || l.specialty.includes(cat)));
      sendJSON(res, active.map(l => ({ name: l.name, city: l.city, phone: l.phone, specialty: l.specialty, experience: l.experience, plan: l.plan })));
      return;
    }
    ...
    if (req.method === 'POST' && url === '/api/lawyer-login') {
      parseBody(req, (err, data) => {
        ...
        const lawyers = readDB('lawyers.json');
        const lawyer = lawyers.find(l => l.access_code === data.code && l.active);
        if (lawyer) sendJSON(res, { success: true, lawyer: { name: lawyer.name, city: lawyer.city, specialty: lawyer.specialty, plan: lawyer.plan, id: lawyer.id } });
        else sendJSON(res, { success: false, error: 'Invalid or inactive access code. Contact WakeelAI support.' });
      });
      return;
    }

    // ── API: lawyer case research ────────────────────────────
    if (req.method === 'POST' && url === '/api/lawyer-research') {
      parseBody(req, (err, data) => {
        if (err) { sendJSON(res, { error: 'Bad request' }); return; }
        // Verify lawyer code
        const lawyers = readDB('lawyers.json');
        const lawyer = lawyers.find(l => l.access_code === data.code && l.active && l.plan === 'premium');
        if (!lawyer) { sendJSON(res, { error: 'Premium access required. Contact WakeelAI to upgrade.' }); return; }
        const researchSystem = [
          'You are an expert Pakistani legal research assistant helping a practicing advocate prepare for court.',
          'You have deep knowledge of Pakistani case law, statutes, and court procedures.',
          '',
          'When given a case description, provide:',
          '1. CASE ANALYSIS — Identify the key legal issues and applicable laws (cite Act names, be careful with section numbers)',
          '2. RELEVANT LAWS — List all applicable Pakistani laws and ordinances',
          '3. LEGAL ARGUMENTS — Strong arguments the advocate can make in court',
          '4. COUNTER-ARGUMENTS — What the opposing side may argue and how to counter',
          ...
          '',
          'Be precise, professional, and practical. This is for a qualified advocate preparing a court case.',
          'Always note: "This research is for informational purposes. The advocate should verify all citations independently."'
        ].join('\n');
        groqRequest(apiKey, researchSystem, [{ role: 'user', content: data.caseDetails }], (text, error) => {
          if (error) sendJSON(res, { error });
          else sendJSON(res, { content: [{ type: 'text', text }], lawyer: lawyer.name });
        });
      });
      return;
    }
    ...
    if (req.method === 'POST' && url === '/api/request-document') {
      parseBody(req, (err, data) => {
        ...
        const newDoc = {
          ...
          price: data.price, status: 'pending', createdAt: new Date().toISOString(),
          document: null
        };
        docs.push(newDoc);
        ...
        sendJSON(res, {
          success: true, id,
          message: 'Request received! Send Rs. ' + data.price + ' to JazzCash: ' + admin.jazzcash + ' with reference: ' + id + '. WhatsApp payment screenshot to: ' + admin.whatsapp
        });
      });
      return;
    }

    // ── API: generate document (admin approves) ──────────────
    if (req.method === 'POST' && url === '/api/generate-document') {
      parseBody(req, (err, data) => {
        if (err) { sendJSON(res, { error: 'Bad request' }); return; }
        const admin = readDB('admin.json');
        if (data.password !== admin.password) { sendJSON(res, { error: 'Wrong password' }); return; }
        const docs = readDB('documents.json');
        ...
        const docSystem = [
          ...
          'For applications and petitions: use proper formal Pakistani legal format.',
          'For agreements: include all necessary clauses for Pakistani law.',
          'Always include: date, parties involved, subject matter, and proper closing.',
          'Write in the language requested (English or Urdu).',
          'IMPORTANT: Be precise and professional. This document may be used in actual legal proceedings.'
        ].join('\n');
        const prompt = 'Please draft a ' + doc.type + ' document for the following situation:\n\n' + doc.description + '\n\nClient name: ' + doc.name + '\n\nMake it complete, professional, and ready to use in Pakistan.';
    ...
    if (req.method === 'POST' && url === '/api/check-document') {
      parseBody(req, (err, data) => {
        if (err) { sendJSON(res, { error: 'Bad request' }); return; }
        const docs = readDB('documents.json');
        const doc = docs.find(d => d.id === data.id && d.phone === data.phone);
    ...
    if (req.method === 'POST' && url === '/api/admin-data') {
      parseBody(req, (err, data) => {
        ...
        const admin = readDB('admin.json');
        if (data.password !== admin.password) { sendJSON(res, { error: 'Wrong password' }); return; }
        sendJSON(res, { lawyers: readDB('lawyers.json'), documents: readDB('documents.json'), admin });
      });
      return;
    }

    // ── API: admin — add/update lawyer ───────────────────────
    ...
    // ── API: admin — update document status ─────────────────
  ...
  server.listen(PORT, () => {
    console.log('\n========================================');
    console.log('  WakeelAI v2 running!');
    console.log('  Main app:     http://localhost:' + PORT);
    console.log('  Lawyer portal: http://localhost:' + PORT + '/lawyer-portal');
    ...
    groqRequest(apiKey, 'You are helpful.', [{ role:'user', content:'Say OK.' }], (t, e) => {
      ...
      else console.log('AI is working! Key is valid.\n');
    });
  });
}
...
else if (process.env.RAILWAY_ENVIRONMENT) { console.log('Set GROQ_API_KEY in Railway variables!'); process.exit(1); }
